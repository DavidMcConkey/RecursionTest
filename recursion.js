...
function everyOther(str, idx = 0, nStr = "") {
  if (idx >= str.length) return nStr;
  nStr += str[idx];
  return everyOther(str, idx + 2, nStr);
}
...
function revString(str, idx = 0, nStr = "") {
  if (nStr.length === str.length) return nStr;
  nStr += str[str.length - 1 - idx];
  return revString(str, idx + 1, nStr);
}
